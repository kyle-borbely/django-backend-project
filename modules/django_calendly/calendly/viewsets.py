...
class CalendlyViewSet(viewsets.GenericViewSet):
    ...
    @property
    def calendly_service(self):
        """
        Property method to get a CalendlyService instance initialized with the current user.
        This uses Django's lazy evaluation to ensure `request.user` is accessed in a request context.
        """
        # Ensure there is a user before proceeding to avoid errors on views without authentication
        if not hasattr(self, '_calendly_service') or self._calendly_service is None:
            if self.request and hasattr(self.request, 'user'):
                self._calendly_service = CalendlyService(user=self.request.user)
            else:
                # Handle cases where request.user is not available; adjust based on your authentication setup
    ...
    def start_auth(self, request):
        ...
        if not request.user.is_authenticated or request.user.user_type != "Coach":
            return Response(
                {"detail": "You are not authorized to access this."},
                status=status.HTTP_403_FORBIDDEN,
            )
        """
        Redirects the user to Calendly's authorization page.
        """
        ...
        scope = 'default'  # Specify the necessary scope
        auth_url = f"https://auth.calendly.com/oauth/authorize?client_id={os.getenv('CALENDLY_CLIENT_ID')}&redirect_uri={os.getenv('CALDENDLY_REDIRECT_URI')}&response_type=code&scope={scope}&state={request.user.id}"
        return Response(data={'redirect_url': auth_url})

    @action(detail=False, methods=['get'], url_path='auth/callback')
    def oauth_callback(self, request):
        code = request.query_params.get('code')
        user_id = request.query_params.get('state')
        if not code or not user_id:
            return Response({'error': 'Missing code or state in the request'}, status=status.HTTP_400_BAD_REQUEST)

        # Prepare the payload for token exchang

        payload = {
            'grant_type': 'authorization_code',
            'code': code,
            'redirect_uri': os.getenv('CALDENDLY_REDIRECT_URI'),
        }
        user = get_object_or_404(User, pk=user_id)
        calendly_service = CalendlyService(user=user)
        token_response = calendly_service.create_access_token(payload)
        if token_response.get('status_code') == 200:
            data = token_response.get('data')
            access_token = data['access_token']
            refresh_token = data['refresh_token']
            expires_in = data['expires_in']  # Calendly specifies the lifetime of the token in seconds

            # Calculate expiration datetime
            expires_at = timezone.now() + datetime.timedelta(seconds=expires_in)

            # Store the tokens in the database
            calendly_token, _ = CalendlyToken.objects.update_or_create(
                user=user,
                defaults={
            ...
            if calendly_token.redirect_url:
                return redirect(calendly_token.redirect_url)

            return Response({'message': 'Token stored successfully.'})
        else:
            return Response({'error': 'Failed to obtain tokens'}, status=token_response.get('status_code'))
